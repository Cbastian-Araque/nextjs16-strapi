...
const styles = {
  ...
  subheading: "mt-4 text-lg md:text-lg lg:text-2xl",
  button: "mt-8 inline-flex items-center justify-center px-6 py-3 text-base font-medium text-black bg-white rounded-md shadow hoder:bg-gray-100 transition-colors",
};
...
export function HeroSection({ data } : { readonly data: {
  ...
  const imageURL = data.imagen?.url.startsWith('http')
    ? data.imagen.url
    : `${STRAPI_BASE_URL}${data.imagen.url}`
  ...
  return (
    <header className={styles.header}>
      ...
      <div className={styles.overlay}>
        ...
        <Link className={styles.button} target={link.isExternal && '_blank'} href={link.href}>
          {link.label}
        </Link>
      </div>
    </header>
